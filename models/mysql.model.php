...
function check_main_car_part_insertion(){
	...
	if (isset($_POST['product-id']) && ( (int)$_POST['product-id']==0)){
		...
		$details 	= mysql_real_escape_string($_POST['description-text']);
...
function check_post_purchase(){
	...
	if (isset($_POST['action']) && ($_POST['action']=='sale')) {
		...
		if ($query){
			$update_stock = mysqli_query($conn, "UPDATE `car_parts_shop`.`actual_stock` SET `quantity` =`quantity`-1 WHERE `actual_stock`.`id` = $product_id;");
...
function get_car_list($q=''){ 
	...
	if ($query = mysqli_query($conn, "SELECT * FROM `car_models`")){		
